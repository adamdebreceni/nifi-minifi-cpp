...
void RESTSender::initialize(core::controller::ControllerServiceProvider* controller, const std::shared_ptr<Configure> &configure) {
  ...
  if (nullptr != configure) {
    ...
    configure->get(Configure::nifi_c2_rest_url, "c2.rest.url", rest_uri_);
    configure->get(Configure::nifi_c2_rest_url_ack, "c2.rest.url.ack", ack_uri_);
    if (configure->get(Configure::nifi_c2_rest_ssl_context_service, "c2.rest.ssl.context.service", ssl_context_service_str)) {
...
void RESTSender::update(const std::shared_ptr<Configure> &configure) {
  std::string url;
  configure->get(Configure::nifi_c2_rest_url, "c2.rest.url", url);
  configure->get(Configure::nifi_c2_rest_url_ack, "c2.rest.url.ack", url);
}
