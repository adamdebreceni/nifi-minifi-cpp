 ...
 */
#ifdef WIN32
char get_separator(bool force_posix) {
  if (!force_posix) {
    return '\\';
  }
  return '/';
...
std::string normalize_path_separators(std::string path, bool force_posix) {
...
int64_t delete_dir(const std::string &path, bool delete_files_recursively) {
...
int create_dir(const std::string& path, bool recursive) {
#ifdef USE_BOOST
...
void addFilesMatchingExtension(const std::shared_ptr<logging::Logger> &logger, const std::string &originalPath, const std::string &extension, std::vector<std::string> &accruedFiles) {
#ifndef WIN32
...
void list_dir(const std::string& dir, std::function<bool(const std::string&, const std::string&)> callback,
                     const std::shared_ptr<logging::Logger> &logger, bool recursive) {
...
std::vector<std::pair<std::string, std::string>> list_dir_all(const std::string& dir, const std::shared_ptr<logging::Logger> &logger,
    bool recursive)  {
...
std::string concat_path(const std::string& root, const std::string& child, bool force_posix) {
  if (root.empty()) {
    return child;
  }
...
std::tuple<std::string /*parent_path*/, std::string /*child_path*/> split_path(const std::string& path, bool force_posix) {
  if (path.empty()) {
...
std::string get_parent_path(const std::string& path, bool force_posix) {
  std::string parent_path;
  std::tie(parent_path, std::ignore) = split_path(path, force_posix);
  return parent_path;
}

std::string get_child_path(const std::string& path, bool force_posix) {
