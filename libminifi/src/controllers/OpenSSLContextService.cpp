...
std::shared_ptr<SSLContextService> SSLContextService::make_shared(const std::string& name, const std::shared_ptr<Configure> &configuration) {
  return std::make_shared<OpenSSLContextService>(name, configuration);
}

bool OpenSSLContextService::configure_ssl_context(SSL_CTX *ctx) {
...
bool OpenSSLContextService::addP12CertificateToSSLContext(SSL_CTX* ctx) const {
...
bool OpenSSLContextService::addPemCertificateToSSLContext(SSL_CTX* ctx) const {
...
std::string OpenSSLContextService::getLatestOpenSSLErrorString() {
...
#ifdef WIN32
bool OpenSSLContextService::addClientCertificateFromSystemStoreToSSLContext(SSL_CTX* ctx) const {
...
#ifdef WIN32
bool OpenSSLContextService::useClientCertificate(SSL_CTX* ctx, PCCERT_CONTEXT certificate) const {
...
bool OpenSSLContextService::addServerCertificatesFromSystemStoreToSSLContext(SSL_CTX* ctx) const {
#ifdef WIN32
  utils::tls::WindowsCertStoreLocation store_location{cert_store_location_};
  HCERTSTORE hCertStore = CertOpenStore(CERT_STORE_PROV_SYSTEM_A, 0, NULL,
                                        CERT_STORE_OPEN_EXISTING_FLAG | CERT_STORE_READONLY_FLAG | store_location.getBitfieldValue(),
...
#ifdef WIN32
void OpenSSLContextService::addServerCertificateToSSLStore(X509_STORE* ssl_store, PCCERT_CONTEXT certificate) const {
  utils::tls::X509_unique_ptr x509_cert = utils::tls::convertWindowsCertificate(certificate);
  if (!x509_cert) {
