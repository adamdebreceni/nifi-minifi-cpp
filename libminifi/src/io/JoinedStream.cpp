...
JoinedStream::JoinedStream(std::vector<std::unique_ptr<io::BaseStream>>&& components)
    : logger_(logging::LoggerFactory<JoinedStream>::getLogger()),
      components_(std::move(components)) {}

void JoinedStream::closeStream() {
...
void JoinedStream::seek(uint64_t offset) {
  std::lock_guard<std::recursive_mutex> lock(file_lock_);
  offset_ = offset;
  file_stream_->clear();
...
int JoinedStream::writeData(std::vector<uint8_t> &buf, int buflen) {
  throw minifi::Exception{ExceptionType::GENERAL_EXCEPTION, "Write is not supported on JoinedStream"};
}
